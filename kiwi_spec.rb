...
feature "Build image" do
  context "Build preparation" do
    ...
    #dirname = '/tmp/kiwi-2012-06-22-12--44--05/'
    `#{SSH} mkdir #{dirname}`
    `scp ./config.xml.template root@#{config['server']}:#{dirname}/config.xml`
    `scp -r ./root root@#{config['server']}:#{dirname}/`
    image_type_to_test= ['xen','vmx', 'oem']
    image_type_to_test.each do |type|
      scenario "Building #{type}" do
        if type == 'xen' 
          flavour = 'xenFlavour'
          build_type = 'vmx'
        else
        ...
        puts `#{SSH} "cd #{dirname} && /usr/sbin/kiwi -b . -d test#{type}build --type #{build_type} --add-profile #{flavour} --logfile test#{type}.log -y"`
        $?.exitstatus.should be == 0
      end
      to_testdrive = ['oem', 'vmx']
      if to_testdrive.include? type 
        scenario "Testdrive #{type}, reboot, check if it survived" do
          if type == 'vmx'
